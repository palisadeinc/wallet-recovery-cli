...
func recoverPrivateKeyV2(decryptor Decryptor, ersLabel, recoveryData []byte) (ellipticCurve string, privateKey []byte, masterChainCode []byte, err error) {
	recData, err := parseRecoveryDataV2(recoveryData)
	if err != nil {
	...
	keyShares, publicKey, err := recoverKeySharesV2(recData, decryptor, ersLabel)
	if err != nil {
...
func parseRecoveryDataV2(encodedRecoveryData []byte) (*recoveryDataV2, error) {
	var recData recoveryDataV2
...
func recoverKeySharesV2(recoveryData *recoveryDataV2, decryptor Decryptor, label []byte) (map[int]math.Scalar, math.Point, error) {
	keyParts, err := validateV2(*recoveryData, nil, nil, nil)
	if err != nil {
