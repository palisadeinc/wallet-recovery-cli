...
func RecoverPrivateKey(recoveryData RecoveryData, ersPrivateKey *rsa.PrivateKey, ersLabel []byte) (ec.Scalar, error) {
	...
	return reconstruct(threshold, sharingType, keyShares)
}
...
func reconstruct(threshold int, sharingType string, shares map[int]ec.Scalar) (ec.Scalar, error) {
	if len(shares) < 2 {
	...
	case secretshare.ShamirSharing.String():
		return shamirReconstruct(threshold, shares), nil
...
func additiveReconstruct(shares map[int]ec.Scalar) ec.Scalar {
	result := shares[0].Field().Zero()
...
func shamirReconstruct(threshold int, shares map[int]ec.Scalar) ec.Scalar {
	return polynomial.InterpolatePlayers(shares[0].Field().Zero(), threshold, shares)
}
