...
func Sign(privateKey PrivateKey, message []byte) []byte {
	...
	sign(signature, privateKey, message, random.Bytes(32))
	return signature
}

func sign(signature, privateKey, message, auxRand []byte) {
	...
	dPrime, err := ec.Secp256k1.Zn().DecodeScalar(privateKey[:len(privateKey)-PublicKeySize])
	if err != nil {
	...
	if !Verify(privateKey[len(privateKey)-PublicKeySize:], message, signature) {
