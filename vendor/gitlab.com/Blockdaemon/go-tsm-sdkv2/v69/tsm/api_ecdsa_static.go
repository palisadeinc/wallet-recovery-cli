...
import (
	...
	"gitlab.com/Blockdaemon/go-tsm-sdkv2/v69/internal/crypto/ecdsa"
...
func ECDSADerivePublicKey(pkixPublicKey, chainCode []byte, derivationPath []uint32) (derivedPublicKey []byte, err error) {
	derivedKey, err := ecdsaDerivePublicKey(pkixPublicKey, chainCode, derivationPath)
	if err != nil {
...
func ecdsaDerivePrivateKey(rawPrivateKey, chainCode []byte, derivationPath []uint32) (ec.Scalar, error) {
	...
	derivedPrivateKey, err := derivekeys.DeriveECDSAPrivateKey(privateKey, chainCode, derivationPath, caching.NoCache())
	if err != nil {
...
func ecdsaDerivePublicKey(pkixPublicKey, chainCode []byte, derivationPath []uint32) (derivekeys.DerivedPublicKey, error) {
	publicKey, err := ecdsaPKIXPublicKeyToPoint(pkixPublicKey)
	if err != nil {
