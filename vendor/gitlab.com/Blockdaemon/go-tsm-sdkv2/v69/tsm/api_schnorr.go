...
type SchnorrAPI interface {
	...
	//     it must be one of "ED-25519" (for Ed25519), "ED-448" (for Ed448) or "secp256k1" (for BIP-340).
...
func (s *schnorrService) CopyKey(ctx context.Context, sessionConfig *SessionConfig, keyID string, curveName string, newThreshold int, desiredKeyID string) (newKeyID string, err error) {
	if keyID != "" {
		if err := validateKeyID(keyID); err != nil {
			return "", toTSMError(err, ErrInvalidInput)
		}
