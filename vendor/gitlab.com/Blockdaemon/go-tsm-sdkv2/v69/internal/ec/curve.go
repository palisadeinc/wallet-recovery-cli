...
func CurveFromZn(zn Field) (Curve, error) {
	for _, c := range pairingCurves {
		if c.E1().Zn().Equals(zn) || c.E2().Zn().Equals(zn) {
			return nil, fmt.Errorf("subgroup of order %s matches pairing curve %s", zn.Modulus().String(), c.Name())
		}
	}
	for _, c := range curves {
		if c.Zn().Equals(zn) {
