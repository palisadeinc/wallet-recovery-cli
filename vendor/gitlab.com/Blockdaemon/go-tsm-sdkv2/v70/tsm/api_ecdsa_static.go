...
// RecoveryID returns data which can be used to recover the public key given the signature and the message.
//
// An ECDSA signature consists of two scalars (r,s), where r is the x-component of a point R=(x,y) on the curve.
// To recover R=g^k from r we need to solve an equation on a form like y^2=x^3+7. This is a quadratic equation with two
// solutions with opposite sign. In a finite field of prime order, this is equivalent to two solutions with opposite
// parity (oddness). The least significant bit of RecoveryID is 0 if the y-component of the R used to sign was the even
// solution to the quadratic equation, and 1 if the parity of the used y-value was odd.
// The least significant bit of recoveryID corresponds to the `yParity` value in EIP-2098.
// The remaining bits of the recoveryID contains z such that R = (x+zN,y) where N is the modulus. Note that for
// secp256k1, z is zero except with negligible probability, but this may not be the case for other curves.
// Once R is recovered, and s, H(m) are known, the public signign key can be recovered from the signature verification
// equation.
...
func ECDSAVerifySignature(jsonPublicKey, messageHash, asn1Signature []byte) error {
	...
	if !publicKey.isECDSA() {
		return fmt.Errorf("verify signature: not an ECDSA public key")
	}

	if !ecdsa.VerifyASN1(publicKey.value, messageHash, asn1Signature) {
		return fmt.Errorf("verify signature: verification failed")
	}
	return nil
}

// ECDSADerivePrivateKey derives a private from the given private key in raw big endian format, chain code and BIP-32 path.
// The result is a new raw private key. Note that only the elliptic curve secp256k1 supports BIP-32 key derivation.
func ECDSADerivePrivateKey(rawPrivateKey, chainCode []byte, derivationPath []uint32) (derivedPrivateKey []byte, err error) {
	derivedKey, err := ecdsaDerivePrivateKey(rawPrivateKey, chainCode, derivationPath)
	if err != nil {
...
// JSON encoded ECDSA public key. Note that only the elliptic curve secp256k1 supports BIP-32 key derivation.
func ECDSADerivePublicKey(jsonPublicKey, chainCode []byte, derivationPath []uint32) (derivedPublicKey []byte, err error) {
	publicKey, _, err := ecdsaDerivePublicKey(jsonPublicKey, chainCode, derivationPath)
	if err != nil {
		return nil, fmt.Errorf("derive public key: %w", err)
	}
	derivedECPublicKey, err := newECPublicKey("ECDSA", publicKey.Curve().Name(), publicKey.Encode())
	if err != nil {
		return nil, fmt.Errorf("derive public key: %w", err)
	}
...
func ECDSADeriveChainCode(jsonPublicKey, chainCode []byte, derivationPath []uint32) (derivedChainCode []byte, err error) {
	_, derivedChainCode, err = ecdsaDerivePublicKey(jsonPublicKey, chainCode, derivationPath)
	if err != nil {
...
func ECDSAValidateRecoveryData(recoveryData []byte, jsonPublicKey []byte, ersPublicKey *rsa.PublicKey, ersLabel []byte) (err error) {
	...
	if !publicKey.isECDSA() {
		return fmt.Errorf("validate recovery data: not an ECDSA public key")
	}
...
func ecdsaDerivePrivateKey(rawPrivateKey, chainCode []byte, derivationPath []uint32) (ec.Scalar, error) {
	privateKey, err := ec.Secp256k1.Zn().DecodeScalar(rawPrivateKey)
	if err != nil {
		return ec.Scalar{}, fmt.Errorf("unable to decode private key: %s", err)
	}
	publicKey := ec.Secp256k1.G().Multiply(privateKey)

	derivedPrivateKey, err := derivekeys.DeriveECDSAPrivateKey(privateKey, publicKey, chainCode, derivationPath, caching.NoCache())
	if err != nil {
...
func ecdsaDerivePublicKey(jsonPublicKey, chainCode []byte, derivationPath []uint32) (ec.Point, []byte, error) {
	publicKey, err := decodeECPublicKey(jsonPublicKey)
	if err != nil {
		return ec.Point{}, nil, fmt.Errorf("unable to decode public key: %s", err)
	}
	if !publicKey.isECDSA() {
		return ec.Point{}, nil, fmt.Errorf("not an ECDSA public key")
	}
	derivedInfo, err := derivekeys.DeriveECDSAPublicKey(publicKey.value, chainCode, derivationPath, caching.NoCache())
	if err != nil {
		return ec.Point{}, nil, fmt.Errorf("derivation error: %s", err)
	}
	return derivedInfo.PublicKey, derivedInfo.ChainCode, nil
}
