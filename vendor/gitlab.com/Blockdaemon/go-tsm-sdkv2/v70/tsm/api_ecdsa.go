...
type ECDSAAPI interface {
	...
	//
	// Output:
	//   - jsonPublicKey: A JSON encoding of an ECDSA public key.
	...
	//   - jsonPublicKey: The public key corresponding to the imported key shares, as a JSON encoded ECSSA public key.
...
func (e *ecdsaService) PublicKey(ctx context.Context, keyID string, derivationPath []uint32) (jsonPublicKey []byte, err error) {
	if err = validateKeyID(keyID); err != nil {
		return nil, toTSMError(err, ErrInvalidInput)
	}
	...
	publicKey, err := newECPublicKey("ECDSA", jsonResponse.Curve, jsonResponse.PublicKey)
	if err != nil {
		return nil, toTSMError(fmt.Errorf("public key: unable to decode public key: %w", err), ErrOperationFailed)
	}
	if !publicKey.isECDSA() {
		return nil, toTSMError(fmt.Errorf("public key: not an ECDSA public key"), ErrOperationFailed)
	}
...
func (e *ecdsaService) ImportKeyShares(ctx context.Context, sessionConfig *SessionConfig, threshold int, wrappedKeyShare []byte, wrappedChainCode []byte, jsonPublicKey []byte, desiredKeyID string) (keyID string, err error) {
	if desiredKeyID != "" {
		if err = validateKeyID(desiredKeyID); err != nil {
			return "", toTSMError(err, ErrInvalidInput)
		}
	...
	if !publicKey.isECDSA() {
		return "", toTSMError(fmt.Errorf("import key shares: not an ECDSA public key"), ErrOperationFailed)
	}
...
func (e *ecdsaService) parseSignResponse(response io.Reader) (*ECDSAPartialSignResult, error) {
	...
	isV1 := true
	var sShare, u, w ec.Scalar
	if jsonResponse.SShare != nil {
	...
	} else if jsonResponse.U != nil && jsonResponse.W != nil {
		isV1 = false
		u, err = curve.Zn().DecodeScalar(jsonResponse.U)
		if err != nil {
			return nil, fmt.Errorf("sign response: unable to decode signature U: %w", err)
		}

		w, err = curve.Zn().DecodeScalar(jsonResponse.W)
		if err != nil {
			return nil, fmt.Errorf("sign response: unable to decode signature W: %w", err)
		}
	} else {
	...
	protocolID := e.node.info.ECDSA.String()
	if protocolID == "DKLS23" {
		protocolID = "ECDSA"
	}
	...
	case "additive":
		if isV1 {
			partialSignature = partialsignature.NewECDSAPartialSignatureV1(protocolID, jsonResponse.PlayerIndex, jsonResponse.Threshold, secretshare.AdditiveSharing, sShare, R, publicKey)
		} else {
			partialSignature = partialsignature.NewECDSAPartialSignatureV2(protocolID, jsonResponse.PlayerIndex, jsonResponse.Threshold, secretshare.AdditiveSharing, u, w, R, publicKey)
		}
	case "shamir":
		if isV1 {
			partialSignature = partialsignature.NewECDSAPartialSignatureV1(protocolID, jsonResponse.PlayerIndex, jsonResponse.Threshold, secretshare.ShamirSharing, sShare, R, publicKey)
		} else {
			partialSignature = partialsignature.NewECDSAPartialSignatureV2(protocolID, jsonResponse.PlayerIndex, jsonResponse.Threshold, secretshare.ShamirSharing, u, w, R, publicKey)
		}
	default:
